...
//    ========================      广度优先遍历开始    ==========================
    ...
    public void BFSTraverse() {
        ...
        for (int i = 0; i < numVertexes; i++) {
            if (!flags[i]) {
                flags[i] = true;
                System.out.print(this.adjList[i].data + " ");
                queue.offer(i);
                //广度优先遍历
                while (!queue.isEmpty()) {
                    int j = queue.poll();
                    EdgeNode node = this.adjList[j].first;
                    //空结点代表邻接单链表结束
                    //将所有邻近未访问结点加入队列
                    while (node!=null){
                        if (!flags[node.adjvex]) {
                            flags[node.adjvex] = true;
                            System.out.print(this.adjList[node.adjvex].data + " ");
